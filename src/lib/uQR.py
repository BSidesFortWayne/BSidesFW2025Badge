...
ALPHA_NUM: bytes = b'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:'
    ...
    def write(self, buffer):
        if self.mode == MODE_NUMBER:
            ...
            for i in range(0, len(self.data), 2):
                ...
                if len(chars) > 1:
                    print(chars[0], chars[1], type(chars[0]), type(chars[1]))
                    buffer.put(
                        ALPHA_NUM.find(str(chars[0])) * 45 +
                        ALPHA_NUM.find(str(chars[1])), 11)
                else:
